...
namespace MetaQuestTrayTool
{
  public class Meta
  {
    ...
    public Meta(string appDataPath, Steam steam)
    {
      ...
      this.m_steam = steam;
      this.m_softwarePathList = new List<string>();
      this.m_manifestFileList = new List<FileInfo>();
      this.m_assetDirectoryList = new List<DirectoryInfo>();
    }
    ...
    private bool TryGetMetaPath(ref string MetaPath)
    {
      ...
      try
      {
        RegistryKey registryKey = Registry.LocalMachine.OpenSubKey("Software\\WOW6432Node\\Oculus VR, LLC\\Oculus", false) 
                               ?? Registry.LocalMachine.OpenSubKey("Software\\Oculus VR, LLC\\Oculus", false);
    ...
    private bool TryGetOculusSoftwarePathList(ref List<string> softwarePathList)
    {
      ...
      try
      {
        string librariesKeyPath = Path.Combine(ownerSid, "SOFTWARE\\Oculus VR, LLC\\Oculus\\Libraries");
        using (RegistryKey registryKey = Registry.Users.OpenSubKey(librariesKeyPath, false))
        {
      ...
      ref List<DirectoryInfo> assetFolderList)
    {
      ...
      try
      {
        ...
        {
            fileInfoList.AddRange(new DirectoryInfo(pathLimit).GetFiles("*.json"));
        }
        
        if (Directory.Exists(pathAssets))
        {
            assetFolderList.AddRange(new DirectoryInfo(pathAssets).GetDirectories("*_assets"));
        }

        foreach (string softwarePath in this.m_softwarePathList)
        {
          string maniPath = Path.Combine(softwarePath, "Manifests");
          if (Directory.Exists(maniPath))
          {
             fileInfoList.AddRange(new DirectoryInfo(maniPath).GetFiles("*.json"));
          }
          
          string assetPath = Path.Combine(softwarePath, "Software\\StoreAssets");
          if (Directory.Exists(assetPath))
          {
            assetFolderList.AddRange(new DirectoryInfo(assetPath).GetDirectories("*_assets"));
          }
        }
      ...
      ref List<string> assetDirectoryList)
    {
      ...
      try
      {
        ...
        return (manifestFileList.Count + assetDirectoryList.Count) > 0;
      }
      catch (Exception ex)
      {
        Log.WriteToLog("TryGetManifestFileNameAndAssetFolderList: " + ex.Message);
