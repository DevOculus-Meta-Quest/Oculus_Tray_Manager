...
namespace OculusTrayTool
{
  internal sealed class GetGames
  {
    ...
    public static void GetThirdPartyApps(string p)
    {
      ...
      if (Operators.CompareString(OculusTrayTool.My.MySettings.Default.LibraryPath, "", false) != 0 & !string.IsNullOrWhiteSpace(OculusTrayTool.My.MySettings.Default.LibraryPath.ToString()))
        otherAssetPath = Path.Combine(Strings.Split(OculusTrayTool.My.MySettings.Default.LibraryPath, ",")[0], "Software\\StoreAssets");
      else
        str1 = oculusPath;
        ...
        try
        {
          if (!str2.Contains("_assets.json"))
          {
            ...
            else
            {
                ...
                foreach (JProperty jproperty in list)
                {
                  ...
                  if (Operators.CompareString(name, "displayName", false) != 0)
                  {
                    if (Operators.CompareString(name, "launchFile", false) == 0)
                    {
                      ...
                      if (withoutExtension.ToLower().EndsWith(".exe") | Operators.CompareString(withoutExtension.ToLower(), "unknown app", false) == 0)
                      {
              ...
              // finally block removed as foreach handles disposal
              if (!(bool) jobject.SelectToken("thirdParty"))
              {
                if (!FrmMain.fmain.includedApps.Contains(str2))
                {
              ...
              else if (Operators.CompareString(withoutExtension, "", false) != 0 & Operators.CompareString(str3, "", false) != 0)
              {
                if (Globals.dbg)
                ...
                if (Operators.CompareString(Left, "", false) != 0)
                {
                  ...
                  else
                  {
                    ...
                    if (num1 > -1 && num2 > -1)
                    {
                      ...
                      if (Operators.CompareString(str11, "", false) != 0 & Operators.CompareString(str6, "", false) != 0)
                      {
